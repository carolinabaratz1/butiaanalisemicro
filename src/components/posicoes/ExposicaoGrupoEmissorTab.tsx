...
const STATUS_CLASS: Record<StatusKey, string> = {
  ...
  "Não aprovado": "bg-red-700/20 text-red-800 dark:text-red-200 border-red-700/40",
};
...
function ExposureCell({
  ...
  const widthPct = pct == null ? 0 : Math.min(100, (pct * 100) / 10); // 10% como referência visual
  return (
...
export function ExposicaoGrupoEmissorTab({ valDate, availableDates, onValDateChange }: Props) {
  ...
  // Aplica filtros sobre emissores. Para o modo grupo, filtramos os emissores
  // e depois reconstruímos os grupos.
  const filteredEmissores = useMemo<EmissorAgg[]>(() => {
    ...
    for (const e of data.porEmissor) {
      ...
      if (comPosicaoFilter === "no" && e.totalButia > 0) continue;

      // Filtros que afetam ativos
      const ativosFiltrados = e.ativos.filter((a) => {
        if (fundFilter !== "all" && a.fundo !== fundFilter) return false;
      ...
      // Recalcular exposições para refletir o filtro de fundo/tipo
      ...
      const totalPL = Object.values(data.plByFundo).reduce((s, v) => s + v, 0);
      out.push({
        ...e,
        ...
        hasTaxa: den > 0,
        consolidatedPct: totalPL ? total / totalPL : null,
      });
    }
  ...
  const diff = data ? Math.abs(sumFiltered - filteredEmissores.reduce((s, e) => s + e.totalButia, 0)) : 0;

  const handleExport = () => {
    if (!data) return;
    const filtersLabel = [
      `Fundo: ${fundFilter === "all" ? "Todos" : fundFilter}`,
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Select value={fundFilter} onValueChange={setFundFilter}>
          <SelectTrigger className="w-56 h-8 text-xs bg-surface-1 border-border">
        ...
        <div className="relative flex-1 min-w-[200px]">
          ...
          <Input
            placeholder="Buscar grupo, emissor, ativo, CNPJ ou ISIN…"
            value={search}
        ...
        <CardContent className="p-0">
          <div className="px-3 py-2 border-b border-border flex items-center justify-between">
            ...
            <div className="text-[10px] text-muted-foreground">
              Soma filtrada: {fmtBRL(sumFiltered)}
              {diff > 1 && ` (diff R$ ${diff.toFixed(2)})`}
            </div>
          </div>
...
function GrupoRow({
  ...
  const isVencido = g.status === "Vencido";
  return (
    <Fragment>
      <TableRow
        className={cn(
          "text-xs",
          isVencido && "bg-red-500/5 border-l-2 border-red-500",
        )}
      >
        ...
        <TableCell>
          <Badge variant="outline" className={cn("text-[10px]", STATUS_CLASS[g.status])}>
            {g.status}
          </Badge>
        </TableCell>
      </TableRow>
...
function EmissorRow({
  ...
  const isOpen = expanded.has(key);
  const isVencido = e.status === "Vencido";
  return (
    <Fragment>
      <TableRow
        className={cn(
      ...
      >
        ...
        <TableCell>
          <Badge variant="outline" className={cn("text-[10px]", STATUS_CLASS[e.status])}>
            {e.status}
          </Badge>
        </TableCell>
      </TableRow>
          ...
          <TableCell colSpan={6 + fundos.length + 4} className="p-0">
            <div className="px-6 py-2">
              <table className="w-full text-[11px]">
                ...
                <tbody>
                  {e.ativos.map((a) => (
                    <tr key={a.posicaoId} className="border-t border-border/40">
                      ...
                      <td className="py-1">
                        <Badge variant="outline" className={cn("text-[9px]", STATUS_CLASS[a.status])}>
                          {a.status}
                        </Badge>
                      </td>
                    </tr>
