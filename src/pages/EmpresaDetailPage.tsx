...
import { emissores, emissoes, type Emissor, type Emissao } from '@/data/emissores';
...
export default function EmpresaDetailPage() {
  ...
  const [solicitarModal, setSolicitarModal] = useState<string | null>(null); // isin
  const [novaEmissaoModal, setNovaEmissaoModal] = useState(false);
  const [entregarModal, setEntregarModal] = useState<string | null>(null); // analise id
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold text-foreground">Emissões ({emissoesList.length})</h3>
        {isGestor && (
