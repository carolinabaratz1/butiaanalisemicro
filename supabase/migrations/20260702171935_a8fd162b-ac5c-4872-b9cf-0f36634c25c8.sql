...
-- ==========================================================================
-- issuer_limits: limites de risco aprovados por emissor
-- ==========================================================================
CREATE TABLE public.issuer_limits (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  cnpj_emissor text NOT NULL,
  grupo_economico text,
  limit_value numeric,
  limit_pct_nav numeric,
  limit_type text NOT NULL DEFAULT 'valor',
  effective_from date,
  effective_to date,
  approved_by text,
  committee_date date,
...
CREATE INDEX idx_issuer_limits_cnpj ON public.issuer_limits(cnpj_emissor);
CREATE INDEX idx_issuer_limits_grupo ON public.issuer_limits(grupo_economico);
...
CREATE POLICY "Authenticated read issuer_limits"
  ON public.issuer_limits FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "Gestor/Risco insert issuer_limits"
  ON public.issuer_limits FOR INSERT
  TO authenticated
  WITH CHECK (
    public.has_role(auth.uid(), 'Gestor'::app_role)
    OR public.has_role(auth.uid(), 'Risco e Compliance'::app_role)
  );

CREATE POLICY "Gestor/Risco update issuer_limits"
  ON public.issuer_limits FOR UPDATE
  TO authenticated
  USING (
    public.has_role(auth.uid(), 'Gestor'::app_role)
    OR public.has_role(auth.uid(), 'Risco e Compliance'::app_role)
...
CREATE POLICY "Gestor/Risco delete issuer_limits"
  ON public.issuer_limits FOR DELETE
  TO authenticated
  USING (
    public.has_role(auth.uid(), 'Gestor'::app_role)
    OR public.has_role(auth.uid(), 'Risco e Compliance'::app_role)
...
-- normaliza CNPJ e atualiza updated_at
...
CREATE TRIGGER trg_issuer_limits_normalize
  BEFORE INSERT OR UPDATE ON public.issuer_limits
  FOR EACH ROW EXECUTE FUNCTION public.tg_issuer_limits_normalize();

-- ==========================================================================
-- get_emissores_gestao: consolidação para tela de Emissores (visão de gestor)
-- ==========================================================================
...
RETURNS TABLE (
  ...
  analise_data_conclusao date,
  analise_data_validade date,
  ...
  funds_list jsonb,
  largest_fund text,
  largest_fund_value numeric,
  largest_fund_pct numeric,
  consolidated_pct numeric,
  limit_value numeric,
  limit_pct_nav numeric,
  limit_type text,
  usage_ratio numeric,
...
BEGIN
  ...
  pos_last AS (
    SELECT pp.fundo, pp.isin, pp.financial_price
    FROM pos_parsed pp
    JOIN last_by_fund lf ON lf.fundo = pp.fundo AND lf.last_d = pp.d
  ),
  fund_pl AS (
    SELECT fundo, SUM(financial_price) AS pl
    FROM pos_last
    GROUP BY fundo
  ),
  pos_issuer AS (
    SELECT
      ...
      pl.fundo,
      SUM(pl.financial_price) AS valor
    FROM pos_last pl
...
GRANT EXECUTE ON FUNCTION public.get_emissores_gestao() TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_emissores_gestao() TO anon;
