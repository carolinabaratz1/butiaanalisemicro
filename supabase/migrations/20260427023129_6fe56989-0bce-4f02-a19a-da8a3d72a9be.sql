-- Drop the old single-parameter overload that conflicts with the new one
DROP FUNCTION IF EXISTS public.get_ipca_history(date);

-- Recreate the canonical version (idempotent — same as previous migration)
