-- ============================================================
-- BUTIA · Função SQL: recalc_trade_metricas
-- Executa server-side após cada upload.
-- Calcula todas as janelas temporais, Z-scores e spreads IPCA.
-- ============================================================

CREATE OR REPLACE FUNCTION recalc_trade_metricas()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  -- Datas de referência (dias úteis pela tabela trade_taxas)
  ...
  SELECT
    ...
    t.pu_curva,
    t.pu_indicativo,
    CASE WHEN t.pu_curva > 0 THEN t.pu_indicativo / t.pu_curva END AS pu_ratio,
    w.avg_5d,  w.avg_10d, w.avg_21d, w.avg_30d, w.avg_90d,
    w.std_90d,
    CASE WHEN w.std_90d > 0 THEN (t.last_val - w.avg_90d) / w.std_90d END AS z_score,
    CASE WHEN w.std_90d > 0 THEN (t.last_val - w.avg_5d)  / w.std_90d END AS z_score_5d,
    CASE WHEN w.std_90d > 0 THEN (t.last_val - w.avg_10d) / w.std_90d END AS z_score_10d,
    CASE WHEN w.std_90d > 0 THEN (t.last_val - w.avg_21d) / w.std_90d END AS z_score_21d,
    (t.last_val - w.first_val) * 100 AS change_bps,
    v.total_qtd,
    v.total_vol_fin,
    NOW()
  FROM (
    -- Última taxa de cada ticker
    SELECT DISTINCT ON (ticker)
      ticker,
      data          AS last_date,
      taxa_indicativa * 100 AS last_val,
      qtd_negociada AS last_qtd,
      vol_financeiro AS last_vol_fin,
      pu_curva,
      pu_indicativo
    FROM trade_taxas
  ...
  JOIN LATERAL (
    ...
    FROM trade_taxas tt
    WHERE tt.ticker = t.ticker
  ) w ON true
  JOIN LATERAL (
    SELECT
      SUM(qtd_negociada)  AS total_qtd,
      SUM(vol_financeiro) AS total_vol_fin
    FROM trade_taxas tv
    WHERE tv.ticker = t.ticker AND tv.data >= v_d90
  ) v ON true
  ON CONFLICT (ticker) DO UPDATE SET
    indexador       = EXCLUDED.indexador,
    last_date       = EXCLUDED.last_date,
    last_val        = EXCLUDED.last_val,
    last_qtd        = EXCLUDED.last_qtd,
    last_vol_fin    = EXCLUDED.last_vol_fin,
    pu_curva        = EXCLUDED.pu_curva,
    pu_indicativo   = EXCLUDED.pu_indicativo,
    pu_ratio        = EXCLUDED.pu_ratio,
    avg_5d          = EXCLUDED.avg_5d,
    avg_10d         = EXCLUDED.avg_10d,
    avg_21d         = EXCLUDED.avg_21d,
    avg_30d         = EXCLUDED.avg_30d,
    avg_90d         = EXCLUDED.avg_90d,
    std_90d         = EXCLUDED.std_90d,
    z_score         = EXCLUDED.z_score,
    z_score_5d      = EXCLUDED.z_score_5d,
    z_score_10d     = EXCLUDED.z_score_10d,
    z_score_21d     = EXCLUDED.z_score_21d,
    change_bps      = EXCLUDED.change_bps,
    total_qtd       = EXCLUDED.total_qtd,
    total_vol_fin   = EXCLUDED.total_vol_fin,
    updated_at      = NOW();

  -- ── Métricas IPCA (spread capitalizado) ───────────────
  INSERT INTO trade_metricas (
    ticker, indexador, last_date, last_val,
    last_qtd, last_vol_fin, pu_curva, pu_indicativo, pu_ratio,
    avg_5d, avg_10d, avg_21d, avg_30d, avg_90d, std_90d,
  ...
  SELECT
    ...
    (SELECT taxa_indicativa * 100 FROM trade_ntnb
     WHERE bond_name = ref.ntnb_ref ORDER BY data DESC LIMIT 1) AS ntnb_taxa,
    NOW()
  FROM (
    SELECT DISTINCT ON (ticker)
      ticker,
  ...
  ON CONFLICT (ticker) DO UPDATE SET
    ...
    avg_21d = EXCLUDED.avg_21d, avg_30d = EXCLUDED.avg_30d, avg_90d = EXCLUDED.avg_90d,
    std_90d = EXCLUDED.std_90d,
    z_score = EXCLUDED.z_score, z_score_5d = EXCLUDED.z_score_5d,
    z_score_10d = EXCLUDED.z_score_10d, z_score_21d = EXCLUDED.z_score_21d,
    change_bps = EXCLUDED.change_bps,
    total_qtd = EXCLUDED.total_qtd, total_vol_fin = EXCLUDED.total_vol_fin,
    ntnb_ref = EXCLUDED.ntnb_ref, ntnb_taxa = EXCLUDED.ntnb_taxa,
    updated_at = NOW();

END;
$$;

GRANT EXECUTE ON FUNCTION recalc_trade_metricas() TO service_role;
